...
import { FakeBookingRepository } from "../../infrasctructure/repositories/fake_booking_repository";
...
describe("BookingService", () => {
  ...
  it("deve lançar um erro quando a propriedade não for encontrada", async () => {
    ...
    await expect(bookingService.createBooking(bookingDTO)).rejects.toThrow("Propiedade não encontrada.");
  });

  it("deve lançar um erro quando o usuário não for encotrado", async () => {
    const mockProperty = {
      getId: jest.fn().mockReturnValue("1"),
    } as any;
  ...
  it("deve lançar uma reserva existente usando o", async () => {
    const mockProperty = {
      getId: jest.fn().mockReturnValue("1"),
      isAvailable: jest.fn().mockReturnValue(true),
      validateGuestCount: jest.fn(),
